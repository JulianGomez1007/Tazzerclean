										...
										paypal.Button.render({
										  ...
										  payment: function (data, actions) {
											return actions.payment.create({
											  transactions: [{
												amount: {
												  total: $("#wallet_amt").val(),
												  currency: $("#currency_val").val()
												}
											  }]
											});
										  },
										  onAuthorize: function (data, actions) {
											return actions.payment.execute()
											  .then(function () 
											  {
													window.location = "<?php echo base_url(); ?>user-wallet-payment/?paymentID="+data.paymentID+"&payerID="+data.payerID+"&token="+data.paymentToken+"&amount="+$("#wallet_amt").val();
											  });
										  }
										}, '#paypal-button');
									}
    ...
    <script>
        $('#stripe_wallet').hide();
		 $('#pay_by_stripe').click(function(){
			 if($("#wallet_amt").val() > 0)
			 {
				 window.location="<?php echo base_url(); ?>pay-by-stripe/"+$("#wallet_amt").val();
			 }
			 else
			 {
